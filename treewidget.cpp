...
TreeWidget::TreeWidget(QWidget *parent) : QWidget(parent){
    ...
    view_1 = new TreeView(this);
    view_1->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    ...
    /*QPushButton**/ next_tree=new QPushButton("Next tree");
    /*QPushButton**/ back_tree=new QPushButton("Previous tree");
...
void TreeWidget::next(){
    ...
    view_->setTree(trees[++stateIndex]);
    if (beginers[treeIndex+1] == stateIndex)
        treeIndex++;
    step->setText(QString::asprintf("Step = %d",treeIndex));
    update();
}
...
void TreeWidget:: back_step(){
    if (dynamic == 1){
        ...
        if(stateIndex<1)return;
            //view_dyn->setTree(states[--stateIndex]->root);
            code->setText(states[stateIndex]->msg);
            step->setText(QString::asprintf("Номер шага = %d",stateIndex));
            update();
    }
